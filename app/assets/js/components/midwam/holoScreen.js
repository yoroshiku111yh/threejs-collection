...
const HoloScreenShader = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
			...
			float direction = 1.;
			if(mod(vUv.y * 1000. + uTime*0.5, 4.) > 2.){
				direction = -1.;
			}
			
			float lineOffset = snoise(vec3(0., 10., uTime*2.0));
			uv.x += direction * (8. + glitchHolo*lineOffset)/uSize.x;
			vec3 holo = texture2D(tDiffuse, uv).rgb;
